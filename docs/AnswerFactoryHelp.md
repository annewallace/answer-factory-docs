...
![Login page image](loginbox.PNG "Login Box for login page")
<br><br><br>
...
___
![Open Project button image](openbutton.PNG "Open Project button") Click to switch to Viewing projects.
___
![Create Project button image](createbutton.PNG "Create Project button") Click to switch to Creating projects.
___
![User Info dropdown image](userinfobase.PNG "User Info Dropdown") Shows the user currently logged in. Click to see the menu options available to the user. There are currently two available options: View your **Account Usage** stats or **Log Out** of AnswerFactory.
___
![Zoom in/out button image](zoombuttons.PNG "Zoom In/Out buttons") Click to zoom in or out on the map. The mouse wheel can also be used to zoom.
___
![Legend icon image](legendicon.PNG "Legend icon") Hover over icon to see the layer list. Layers may be toggled on and off, and basemap may be changed here. There are currently two possible basemaps.
___
![Help button image](helpicon.PNG "Help icon") Click to reach the help pages.
___
<br><br>
...
![Open Project page layout image](openproject-pagelayout.PNG "Open Project page layout") <br>
<br>
___
![Show my projects toggle image](myprojectstoggle.PNG "Show my projects toggle") Toggle on viewing solely projects you have created, or leave toggled off to view all projects created in your group account.
___
![minimize button image](minimizeicon.PNG "Minimize button") Click to minimize the window. Click again to bring the window back to full view.
___
![Refresh button image](refreshbutton.PNG "Refresh button") Click to force refresh the project list manually. Project list automatically updates once a minute.
___
![Search bar image](searchbar.PNG "Search Bar") Type text to dynamically filter project list by project name.
___
![Project list page navigation image](pagenavigation.PNG "Project list page Navigation") There are several ways to move through the pages of the list of projects in your account. Click on the arrows to jump forward or back through pages. Click on the visible numbers to jump to that page. Click on the dropdown to select any specific page to jump to.
___
<br><br>
...
<br>
![Viewing Project page layout image](openproject-pagelayout2.PNG "Viewing a project page layout")
<br><br>

####Project Information####
![Project Info image](projectinfo.PNG "Project information window")
<br>
...
___
![Return to Project List icon image](returntolist.PNG "Return to project list icon") Click to return to the list of projects.
___
![minimize button image](minimizeicon.PNG "Minimize button") Click to minimize the window. Click again to bring the window back to full view.
___
![Remove answer button image](removeanswer.PNG "Remove answer button") Click to remove an answer from the project. *Note: you must click the Update button to update the project after altering it.*
___
...
___
![Add Answer button image](addanswerbutton.PNG "Add Answer button") Click to add the selected answer to the project. Answers added to the project will appear on the right of the screen.
___
![Delete project button image](deletebutton.PNG "Delete project button") Click to delete project from AnswerFactory.
___
![Update button image](updatebutton.PNG "Update button") Click to save any changes made to the project.
___
<br>

####Answers####
![Answers List image](answerslist.PNG "Answers List")
<br>
...
<br>
![Answer table image](answertable.PNG "Answer Table")
<br>
...
<br>
![Table with answers displayed on map image](viewanswers.PNG "Answers viewed in both table and on map")
<br>
You can hover over vectors on the map to view metadata.
<br>
___
![Item Type button image](itemtypes.PNG "Item types button example") When the results have different types, the various types display in the table. Click on a specific type to see only those results in the table, or click on total to see all of them.
___
![Column visibility column options image](columnoptions.PNG "Column visibility column options") Click on column visibility to display the column options, and click on the options to turn them on or off.
___
![Copy button image](copybutton.PNG "Copy button") Click to copy table row(s) to clipboard.
___
![Print button image](printbutton.PNG "Print button") Click to print table.
___
![CSV button image](csvbutton.PNG "CSV button") Click to save table in CSV format.
___
![PDF button image](pdfbutton.PNG "PDF button") Click to save table in PDF format.
___
![Table entries display option image](tableentryreturn.PNG "Number of entries displayed in the table") Click the dropdown to select the number of rows visible in the table at a time.
___
![Table search bar image](tablesearch.PNG "Table search bar") Type to dynamically search the table when looking for a specific aoi.
___
![Show All Results button image](showallbutton.PNG "Show All Results button") Click to display all the results for the given answer for the project over all of the aois.
___
![Table page navigation image](tablenavigation.PNG "Table page navigation") Click on the arrows to move between pages, or click on a specific page number to jump to the page.
___
<br><br>

##Create Project##
###Create Project Page Layout###
![Create Project page layout image](createproject-pagelayout.PNG "Create Project page layout") <br>
<br>
___
![minimize button image](minimizeicon.PNG "Minimize button") Click to minimize the window. Click again to bring the window back to full view.
___
![Upload shapefile button image](uploadbutton.PNG "Upload shapefile button") Click to open your computer directory to upload a **zipped** shapefile (including all necessary components of .dbf, .prj, .shp, and .shx); this shapefile will serve as your aoi, and the map will automatically snap to the shapefile once upload is complete. Aoi(s) display in *pink*.
___
![Draw button image](draw.PNG "Draw button") Click to activate draw capability. Single click on map to drop the vertices of your aoi, and double-click to complete an aoi. Draw remains active, and you may draw any number of aois. Once you have completed drawing aois, click Draw button again to deactivate continued drawing capability. Aoi(s) display in *pink*.
___
<br>
Once an aoi is added or created, more functionality is exposed. <br>
![Create Project with aoi present page layout image](createproject-pagelayout2.PNG "Create Project with aoi present page layout") <br>
<br>
___
...
___
![Compute button image](computebutton.PNG "Compute button") Click to compute and add the buffer surrounding the aoi(s). The buffer will display in *orange*.
___
...
![Add Answer button image](addanswerbutton.PNG "Add Answer button") Click to add the selected answer to the project. Answers added to the project will appear on the right of the screen.
___
![Reset project button image](resetbutton.PNG "Reset project button") Click to clear all layers and answers to start over from scratch.
___
![Save button image](savebutton.PNG "Save button") Click to save the project once you have set all of your specifications.
___
<br><br>
